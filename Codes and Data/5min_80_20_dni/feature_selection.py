...
irradiance_5m = irradiance[['timestamp','B(dni_kt|5min)', 'V(dni_kt|5min)', 'L(dni_kt|5min)']]
sky_image_5m = sky_image.drop(columns=['AVG(NRB)', 'STD(NRB)', 'ENT(NRB)', 'timestamp'])
target_5m = target[['dni_5min','dni_5min','dni_clear_5min', 'dni_clear_5min','dni_kt_5min','dni_kt_5min']]
...
features_train, features_test, target_train, target_test = train_test_split(df_5m.iloc[:, 0:-6], df_5m[['dni_kt_5min','dni_clear_5min','dni_5min']], test_size=0.2, shuffle=False)
...
n_scores = cross_val_score(pipeline, features_train, target_train['dni_kt_5min'],
...
#fitting the model
rfe.fit(features_train, target_train['dni_kt_5min'])
