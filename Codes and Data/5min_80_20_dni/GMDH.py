...
#------------------------------------------------------------------------------
model_GMDH = Combi()
model_GMDH.fit(np.array(features_train), np.array(target_train['dni_kt_5min']))
GMDH_hat_5min = model_GMDH.predict(features_test)

#Conversão em inrradiância utilizando a irradiância de céu claro
GMDH_hat_5min = GMDH_hat_5min * target_test['dni_clear_5min']

#Avaliação do desempenho: RMSE, MAE, R², MAPE
test_GMDH_error = calculate_errors(GMDH_hat_5min, target_test['dni_5min'].values, 1)
