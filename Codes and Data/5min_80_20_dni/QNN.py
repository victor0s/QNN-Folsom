...
#X_train = np.array(features_train[['B(dni_kt|5min)', 'V(dni_kt|5min)', 'L(dni_kt|5min)']]).reshape(len(features_train), 3)
...
y_train = np.array(target_train['dni_kt_5min'])

X_test = np.array(features_test).reshape(len(features_test),  len(features_test.iloc[0]))
y_test = np.array(target_test['dni_kt_5min'])
...
VQR_hat_5min = VQR_hat_5min * target_test['dni_clear_5min']

test_VQR_error = calculate_errors(VQR_hat_5min.values, target_test['dni_5min'].values, 1)
