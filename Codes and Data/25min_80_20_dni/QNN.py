...
features_train =  pd.read_csv("C:/Users/volive04/Desktop/UoG/24WINTER/Quantum Computing/Project/Codes_RFE/25min_80_20_dni/features_train.csv")
features_test = pd.read_csv("C:/Users/volive04/Desktop/UoG/24WINTER/Quantum Computing/Project/Codes_RFE/25min_80_20_dni/features_test.csv")
target_train =  pd.read_csv("C:/Users/volive04/Desktop/UoG/24WINTER/Quantum Computing/Project/Codes_RFE/25min_80_20_dni/target_train.csv")
target_test = pd.read_csv("C:/Users/volive04/Desktop/UoG/24WINTER/Quantum Computing/Project/Codes_RFE/25min_80_20_dni/target_test.csv")
...
#X_train = np.array(features_train[['B(dni_kt|25min)', 'V(dni_kt|25min)', 'L(dni_kt|25min)']]).reshape(len(features_train), 3)
...
y_train = np.array(target_train['dni_kt_25min'])

X_test = np.array(features_test).reshape(len(features_test),  len(features_test.iloc[0]))
y_test = np.array(target_test['dni_kt_25min'])
...
VQR_hat_25min = regressor.predict(X_test)
VQR_hat_25min = np.squeeze(VQR_hat_25min)

VQR_hat_25min = VQR_hat_25min * target_test['dni_clear_25min']

test_VQR_error = calculate_errors(VQR_hat_25min.values, target_test['dni_25min'].values, 1)
