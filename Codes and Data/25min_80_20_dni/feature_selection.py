...
irradiance_25m = irradiance[['timestamp','B(dni_kt|25min)', 'V(dni_kt|25min)', 'L(dni_kt|25min)']]
sky_image_25m = sky_image.drop(columns=['AVG(NRB)', 'STD(NRB)', 'ENT(NRB)', 'timestamp'])
target_25m = target[['dni_25min','dni_25min','dni_clear_25min', 'dni_clear_25min','dni_kt_25min','dni_kt_25min']]
...
features_train, features_test, target_train, target_test = train_test_split(df_25m.iloc[:, 0:-6], df_25m[['dni_kt_25min','dni_clear_25min','dni_25min']], test_size=0.2, shuffle=False)
...
n_scores = cross_val_score(pipeline, features_train, target_train['dni_kt_25min'],
...
#fitting the model
rfe.fit(features_train, target_train['dni_kt_25min'])
