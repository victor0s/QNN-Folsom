...
#------------------------------------------------------------------------------
model_GMDH = Combi()
model_GMDH.fit(np.array(features_train), np.array(target_train['ghi_kt_25min']))
GMDH_hat_25min = model_GMDH.predict(features_test)

#Conversão em inrradiância utilizando a irradiância de céu claro
GMDH_hat_25min = GMDH_hat_25min * target_test['ghi_clear_25min']

#Avaliação do desempenho: RMSE, MAE, R², MAPE
test_GMDH_error = calculate_errors(GMDH_hat_25min, target_test['ghi_25min'].values, 1)
