...
features_train =  pd.read_csv("C:/Users/volive04/Desktop/UoG/24WINTER/Quantum Computing/Project/Codes_RFE/10min_80_20/features_train.csv")
features_test = pd.read_csv("C:/Users/volive04/Desktop/UoG/24WINTER/Quantum Computing/Project/Codes_RFE/10min_80_20/features_test.csv")
target_train =  pd.read_csv("C:/Users/volive04/Desktop/UoG/24WINTER/Quantum Computing/Project/Codes_RFE/10min_80_20/target_train.csv")
target_test = pd.read_csv("C:/Users/volive04/Desktop/UoG/24WINTER/Quantum Computing/Project/Codes_RFE/10min_80_20/target_test.csv")
...
#X_train = np.array(features_train[['B(ghi_kt|10min)', 'V(ghi_kt|10min)', 'L(ghi_kt|10min)']]).reshape(len(features_train), 3)
...
best_weights = None
best_obj_func_eval = float('inf')
patience = 5  #maximum patience
wait = 0

def callback_graph(weights, obj_func_eval):
    global best_weights, best_obj_func_eval, wait
    ...
    #checking the objective function value and updating it if condition is met
    if abs(obj_func_eval - best_obj_func_eval) > 10E-4: #minimum improvement
        best_obj_func_eval = obj_func_eval
        best_weights = weights
        wait = 0  #reseting counter
    else:
        wait += 1  #increasing patience counter

    #breaking loop
    if wait >= patience:
        print(f"Early stopping at iteration {len(objective_func_vals)}")
        return True  # This will cause L-BFGS-B to stop
...
VQR_hat_10min = regressor.predict(X_test)
VQR_hat_10min = np.squeeze(VQR_hat_10min)

VQR_hat_10min = VQR_hat_10min * target_test['ghi_clear_10min']

test_VQR_error = calculate_errors(VQR_hat_10min.values, target_test['ghi_10min'].values, 1)
