...
irradiance_20m = irradiance[['timestamp','B(dni_kt|20min)', 'V(dni_kt|20min)', 'L(dni_kt|20min)']]
sky_image_20m = sky_image.drop(columns=['AVG(NRB)', 'STD(NRB)', 'ENT(NRB)', 'timestamp'])
target_20m = target[['dni_20min','dni_20min','dni_clear_20min', 'dni_clear_20min','dni_kt_20min','dni_kt_20min']]
df_20m = pd.concat([irradiance_20m, sky_image_20m,target_20m], axis = 1).dropna() #merging the features and the target df for 5 min and removing NAs
df_20m = df_20m.drop(columns = ['timestamp'])
#splitting training and validation datasets randomly: kt_dni
features_train, features_test, target_train, target_test = train_test_split(df_20m.iloc[:, 0:-6], df_20m[['dni_kt_20min','dni_clear_20min','dni_20min']], test_size=0.2, shuffle=False)
...
n_scores = cross_val_score(pipeline, features_train, target_train['dni_kt_20min'],
...
#fitting the model
rfe.fit(features_train, target_train['dni_kt_20min'])
