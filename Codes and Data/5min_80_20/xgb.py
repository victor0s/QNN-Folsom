...
tuning_model.fit(features_train.iloc[:,:], target_train.iloc[:,:]['ghi_kt_5min'])
...
# xgboost_reg.fit(X_val_5m, y_val_5m['ghi_kt_5min'])

# y_pred_val_XGB = xgboost_reg.predict(X_val_5m)

# val_XGB_error = calculate_errors(y_pred_val_XGB * y_val_5m['ghi_clear_5min'].values, y_val_5m['ghi_5min'].values, 1)

xgboost_reg.fit(features_test, target_test['ghi_kt_5min'])

y_pred_test_XGB = xgboost_reg.predict(features_test)

test_XGB_error = calculate_errors(y_pred_test_XGB * target_test['ghi_clear_5min'].values, target_test['ghi_5min'].values, 1)
