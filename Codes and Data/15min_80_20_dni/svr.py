...
tuning_model.fit(features_train.iloc[:,:], target_train.iloc[:,:]['dni_kt_15min'])
...
# xgboost_reg.fit(X_val_10m, y_val_10m['dni_kt_15min'])

# y_pred_val_XGB = xgboost_reg.predict(X_val_10m)

# val_XGB_error = calculate_errors(y_pred_val_XGB * y_val_10m['dni_clear_15min'].values, y_val_10m['dni_15min'].values, 1)

svr_model.fit(features_test, target_test['dni_kt_15min'])

y_pred_test_XGB = svr_model.predict(features_test)

test_XGB_error = calculate_errors(y_pred_test_XGB * target_test['dni_clear_15min'].values, target_test['dni_15min'].values, 1)
