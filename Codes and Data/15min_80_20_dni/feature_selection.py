...
irradiance_15m = irradiance[['timestamp','B(dni_kt|15min)', 'V(dni_kt|15min)', 'L(dni_kt|15min)']]
sky_image_15m = sky_image.drop(columns=['AVG(NRB)', 'STD(NRB)', 'ENT(NRB)', 'timestamp'])
target_15m = target[['dni_15min','dni_15min','dni_clear_15min', 'dni_clear_15min','dni_kt_15min','dni_kt_15min']]
...
features_train, features_test, target_train, target_test = train_test_split(df_15m.iloc[:, 0:-6], df_15m[['dni_kt_15min','dni_clear_15min','dni_15min']], test_size=0.2, shuffle=False)
...
n_scores = cross_val_score(pipeline, features_train, target_train['dni_kt_15min'],
...
#fitting the model
rfe.fit(features_train, target_train['dni_kt_15min'])
