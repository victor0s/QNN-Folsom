...
irradiance_30m = irradiance[['timestamp','B(dni_kt|30min)', 'V(dni_kt|30min)', 'L(dni_kt|30min)']]
sky_image_30m = sky_image.drop(columns=['AVG(NRB)', 'STD(NRB)', 'ENT(NRB)', 'timestamp'])
target_30m = target[['dni_30min','dni_30min','dni_clear_30min', 'dni_clear_30min','dni_kt_30min','dni_kt_30min']]
df_30m = pd.concat([irradiance_30m, sky_image_30m,target_30m], axis = 1).dropna() #merging the features and the target df for 5 min and removing NAs
df_30m = df_30m.drop(columns = ['timestamp'])
#splitting training and validation datasets randomly: kt_dni
features_train, features_test, target_train, target_test = train_test_split(df_30m.iloc[:, 0:-6], df_30m[['dni_kt_30min','dni_clear_30min','dni_30min']], test_size=0.2, shuffle=False)
...
n_scores = cross_val_score(pipeline, features_train, target_train['dni_kt_30min'],
...
#fitting the model
rfe.fit(features_train, target_train['dni_kt_30min'])
